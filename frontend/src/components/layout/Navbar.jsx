...
import { HardHat, ShieldCheck, UserCheck, LogOut, ArrowRightLeft, HelpCircle } from 'lucide-react';
import { Badge } from '../ui/Badge';
import { apiClient } from '../../services/api';
import { PlatformExplainerModal } from '../common/PlatformExplainerModal';
    ...
    <>
      <header className="sticky top-0 z-40 bg-slate-900/95 backdrop-blur border-b border-slate-800 shadow-stripe">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          {/* Right actions: Guide modal, Role switch & user menu */}
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            {/* Guide Explainer Button */}
            <button
              onClick={() => setShowGuideModal(true)}
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-xl text-xs font-bold transition-all cursor-pointer"
              title="How BuildGuard AI Works"
            >
      ...
      {/* In-App Explainer Guide Modal */}
      <PlatformExplainerModal
